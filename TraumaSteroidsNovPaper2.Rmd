...
knitr::opts_chunk$set(fig.path = "/Users/lxb732/Desktop/TraumaSteroids/TraumaSteroids_Laura_6Dec3/")
...
```{r Fig1ATrauma, fig.cap = "Figure.1",  fig.height= 4, fig.width= 7}
...
```{r Fig4Trauma, fig.cap = "Figure.4", fig.height=7.5, fig.width= 7}
...
if ((mean(filter(PlotGH, Label == "Healthy")$var)) <= (mean(filter(PlotGH, Label == "Trauma")$var ,na.rm=TRUE))){
  ...
  PlotGH <- mutate(PlotGH, Unhealthy = ifelse(var >= as.numeric((quantile(filter(PlotGH, Label == "Healthy")$var,probs=c(.05,.95))[2])),"yes","no"))   
  ...
  resulta <- GAMModel(PlotGH,i)
    ...
    PlotGH <- mutate(PlotGH, Unhealthy = ifelse(var <= as.numeric((quantile(filter(PlotGH, Label == "Healthy")$var,probs=c(.05,.95))[1])),"yes","no"))   
...
write.csv(Angela, "/Users/lxb732/Desktop/TraumaSteroids/TraumaSteroids_Laura_6Dec3/AngelaStartingTimesFinal.csv", row.names = FALSE)
